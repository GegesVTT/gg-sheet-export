...
import { buildCardsBody, buildCardsPrintHTML, themeStyle, CARDS_CSS, fitSpellCards, THEMES, registerCardTheme, buildThemeTiles, resolveThemeId } from "./spellcards-print.mjs";
...
function spellCardTheme() {
  let id;
  try { id = game.settings.get(MODULE_ID, "spellCardTheme") || "cronicas"; }
  catch (e) { id = "cronicas"; }
  return resolveThemeId(id);   // premium bloqueado / desinstalado -> cae a free
}
...
Hooks.once("init", () => {
  ...
  const themeChoices = () => Object.fromEntries(
    Object.values(THEMES).filter((t) => !t.premium && !t.soon).map((t) => [t.id, t.name])
  );
